...
import com.example.han2.Atask.SideRecipe;
import com.example.han2.Dto.WeekRecipeDTO;
import com.example.han2.R;

import java.util.ArrayList;

public class KindList1 extends Fragment {


    SideRecipe sideRecipe;
    SideRecipeAdapter adapter;
    ...
    public View onCreateView(@NonNull final LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_side_recipe, container, false);
        ...
        //레시피 데이터받기
        ArrayList<WeekRecipeDTO> SideArrayList = new ArrayList<>();
        adapter = new SideRecipeAdapter(SideArrayList);
        recyclerView = view.findViewById(R.id.recyclerView_side_recipe);
        LinearLayoutManager layoutManager1 = new LinearLayoutManager(getContext(), RecyclerView.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager1);
        recyclerView.setAdapter(adapter);
        recyclerView.setNestedScrollingEnabled(false);
        sideRecipe = new SideRecipe(SideArrayList, adapter);
        sideRecipe.execute();
