...
import com.example.han2.Atask.MonthRecipe;
import com.example.han2.Dto.WeekRecipeDTO;
import com.example.han2.R;

import java.util.ArrayList;

public class RankMenu2 extends Fragment {
    MonthRecipe monthRecipe;
    MonthRecipeAdapter adapter;
    ...
    public View onCreateView(@NonNull final LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_month_recipe_rank, container, false);
        ...
        //바텀바 이달의 레시피 데이터받기
        ArrayList<WeekRecipeDTO> monthArrayList = new ArrayList<>();
        adapter = new MonthRecipeAdapter(monthArrayList);
        recyclerView = view.findViewById(R.id.recyclerView_month_recipe_rank);
        LinearLayoutManager layoutManager1 = new LinearLayoutManager(getContext(), RecyclerView.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager1);
        recyclerView.setAdapter(adapter);
        recyclerView.setNestedScrollingEnabled(false);
        monthRecipe = new MonthRecipe(monthArrayList, adapter);
        monthRecipe.execute();
