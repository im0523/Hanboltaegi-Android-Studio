...
import com.example.han2.Atask.SoupRecipe;
import com.example.han2.Dto.WeekRecipeDTO;
import com.example.han2.R;

import java.util.ArrayList;

public class KindList2 extends Fragment {


    SoupRecipe soupRecipe;
    SoupRecipeAdapter adapter;
    ...
    public View onCreateView(@NonNull final LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_soup_recipe, container, false);
        ...
        //레시피 데이터받기
        ArrayList<WeekRecipeDTO> SoupArrayList = new ArrayList<>();
        adapter = new SoupRecipeAdapter(SoupArrayList);
        recyclerView = view.findViewById(R.id.recyclerView_soup_recipe);
        LinearLayoutManager layoutManager1 = new LinearLayoutManager(getContext(), RecyclerView.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager1);
        recyclerView.setAdapter(adapter);
        recyclerView.setNestedScrollingEnabled(false);
        soupRecipe = new SoupRecipe(SoupArrayList, adapter);
        soupRecipe.execute();
