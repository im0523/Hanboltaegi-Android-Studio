...
import com.example.han2.Atask.BbangRecipe;
import com.example.han2.Dto.WeekRecipeDTO;
import com.example.han2.R;

import java.util.ArrayList;

public class KindList8 extends Fragment {


    BbangRecipe bbangRecipe;
    BbangRecipeAdapter adapter;
    ...
    public View onCreateView(@NonNull final LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_bbang_recipe, container, false);
        ...
        //레시피 데이터받기
        ArrayList<WeekRecipeDTO> BbangArrayList = new ArrayList<>();
        adapter = new BbangRecipeAdapter(BbangArrayList);
        recyclerView = view.findViewById(R.id.recyclerView_bbang_recipe);
        LinearLayoutManager layoutManager1 = new LinearLayoutManager(getContext(), RecyclerView.VERTICAL, false);
        recyclerView.setLayoutManager(layoutManager1);
        recyclerView.setAdapter(adapter);
        recyclerView.setNestedScrollingEnabled(false);
        bbangRecipe = new BbangRecipe(BbangArrayList, adapter);
        bbangRecipe.execute();
